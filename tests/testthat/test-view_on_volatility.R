...
vol <- apply(ret_mtx, 2, sd)

vmean_ts <- view_on_volatility(x = ret_ts, vol = vol)
test_that("`view_on_volatility` works for ts", {
...
vmean_mtx <- view_on_volatility(x = ret_mtx, vol = vol)
test_that("`view_on_volatility` works for matrix", {
...
vmean_xts <- view_on_volatility(x = ret_xts, vol = vol)
test_that("`view_on_volatility` works for xts", {
...
vmean_tbl <- view_on_volatility(x = ret_tbl, vol = vol)
test_that("`view_on_volatility` works for tbl_df", {
